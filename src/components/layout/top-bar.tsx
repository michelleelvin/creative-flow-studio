...
export function TopBar() {
  ...
  const user = role === "employee" ? currentEmployee : role === "manager" ? currentManager : currentAdmin;
      ...
      <div className="ml-auto flex items-center gap-2">
        {(() => {
          ...
          return (
            <div className={`hidden sm:flex items-center gap-2 h-8 px-2.5 rounded-full border text-xs ${over ? "border-destructive text-destructive" : ""}`}>
              ...
              <span className="font-mono">{display}{over ? " over" : meta.limitMs ? " left" : ""}</span>
            </div>
          );
        })()}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            <DropdownMenuItem className="gap-2 text-destructive focus:text-destructive"><LogOut className="size-3.5" /> Log out</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>

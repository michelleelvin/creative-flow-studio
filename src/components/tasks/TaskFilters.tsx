...
import { type MyPriority } from "@/data/myTasks";
...
export function TaskFilters({
  ...
  const toggleProject = (id: string) => {
    onChange({
      ...value,
      projectIds: value.projectIds.includes(id)
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="outline" size="sm" className="gap-2">
          ...
          Filter
          {count > 0 && (
            <span className="ml-0.5 inline-flex items-center justify-center rounded-full bg-primary text-primary-foreground text-[10px] font-semibold size-4">
              {count}
            </span>
          )}
        </Button>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {DEADLINES.map((d) => (
              <button
                ...
                className={`text-xs rounded-md px-2 py-1.5 border text-left ${
